...
export interface ProductVariant {
  ...
  stockQuantity: number; // Stock in units or kg
  isActive: boolean;
  sortOrder: number;
  createdAt?: string;
}
...
export interface PrepItemSummary {
  ...
  inStock: number; // current store stock
  deficit: number; // deficit if totalRequired > inStock
  stockStatus: 'sufficient' | 'warning' | 'deficit'; // كافي / قارب على النفاذ / عجز
  ordersCount: number;
