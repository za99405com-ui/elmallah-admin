...
export const OverviewTab: React.FC = () => {
  ...
  const deficitCount = todayPrep.prepItems.filter((i) => i.deficit > 0).length;

  return (
    <div className="space-y-4 sm:space-y-5 pb-8">
      {/* 3:00 AM Cutoff & Kitchen Prep Daily Banner */}
      <div className="bg-gradient-to-r from-blue-950/70 via-slate-900 to-cyan-950/70 border border-blue-800/40 rounded-2xl p-3.5 sm:p-4 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-bold text-xs sm:text-sm text-slate-100 flex items-center gap-1.5">
                <span>تجهيز ومخزون المطبخ:</span>
                <span className="text-cyan-300 font-mono">دورة {todayPrep.prepDateLabel}</span>
              </span>
              ...
              {deficitCount > 0 && (
                <span className="bg-rose-500/20 text-rose-300 text-[10px] font-bold px-2 py-0.5 rounded-full border border-rose-500/30 flex items-center gap-1">
                  ...
                  عجز في {deficitCount} أصناف
                </span>
              )}
            </div>
            <p className="text-[11px] text-slate-400 mt-0.5">
              إجمالي المطلوب لليوم: <strong className="text-cyan-300 font-mono font-bold">{todayPrep.totalKg} كجم</strong> و{' '}
              <strong className="text-cyan-300 font-mono font-bold">{todayPrep.totalPieces} قطعة</strong> لـ{' '}
              <strong className="text-slate-200">{todayPrep.totalOrdersCount} طلب</strong> • الإغلاق القادم بعد {cutoffInfo.hoursUntilCutoff} س و {cutoffInfo.minutesUntilCutoff} د
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>كشف التجهيز والنواقص للمطبخ</span>
          <span>&larr;</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <p className="text-slate-500 dark:text-slate-400 text-[11px] sm:text-xs font-semibold">الأسماك</p>
          ...
          <div className="mt-1.5 flex items-baseline gap-1">
            ...
            <span className="text-[10px] sm:text-xs font-medium text-slate-400">صنف متوفر</span>
          </div>
          <div className="mt-1 text-[10px] sm:text-xs text-slate-400 flex items-center justify-between">
            <span>طازج</span>
            {outOfStockProducts.length > 0 ? (
              <span className="text-rose-500 font-bold">{outOfStockProducts.length} نفد</span>
            ) : (
              <span className="text-emerald-500 font-bold">الكل متاح</span>
            )}
          </div>
        </div>
      </div>
