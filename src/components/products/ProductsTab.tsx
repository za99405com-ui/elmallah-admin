...
export const ProductsTab: React.FC = () => {
  const { products, categories, addProduct, updateProduct, deleteProduct, toggleProductStock, addToast } = useApp();
  ...
  const [variants, setVariants] = useState<Array<{ id?: string; title: string; weightKg: number; pieceCount: number; price: number; stockQuantity: number }>>([]);
      ...
      <div className="p-3 sm:p-3.5 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-cyan-700 dark:text-cyan-300 font-semibold">
            متجر أسماك الملاح يبيع حصرياً الأسماك الطازجة (نيئة بالوزن أو بالعدد) دون طهي.
          </span>
        </div>
          ...
          <table className="w-full text-right border-collapse">
            <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 dark:text-slate-400 text-[11px] font-semibold border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="p-2.5 sm:p-3">السعر والمخزون</th>
                <th className="p-2.5 sm:p-3">التوفر</th>
                          ...
                          {product.variants && product.variants.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {product.variants.map((v) => (
                                <span
                                  key={v.id || v.title}
                                  className="text-[9px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800/80 px-1.5 py-0.5 rounded"
                                >
                                  {v.title}: <strong className="text-cyan-600 dark:text-cyan-400">{v.price} ج.م</strong> ({v.stockQuantity} متبقي)
                                </span>
                              ))}
                            </div>
                          )}
          ...
          <div className="bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-4 sm:p-6 space-y-4 text-slate-900 dark:text-slate-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1">إجمالي المخزون الأساسي</label>
                  <input
                    type="number"
                    min="0"
                    required
              ...
              <div className="p-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 space-y-2.5">
                ...
                {variants.length === 0 ? (
                  <p className="text-[11px] text-slate-400 text-center py-2">
                    لا توجد خيارات مخصصة لهذا الصنف (يتم الاعتماد على السعر والمخزون الأساسي أعلاه). انقر على الزر لإضافة أحجام مثل (وسط 4-6 قطع/كجم، كبير جامبو 2 قطعة/كجم).
                  </p>
                ) : (
                    ...
                    {variants.map((v, idx) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-4 gap-1.5 text-[10px]">
                          <div>
                            <span className="text-slate-400 block mb-0.5">الوزن (كجم)</span>
                            <input
                              type="number"
                              step="0.1"
                              value={v.weightKg}
                          ...
                          <div>
                            <span className="text-slate-400 block mb-0.5">عدد القطع</span>
                            <input
                              type="number"
                              value={v.pieceCount}
