...
export const api = {
  ...
  // Inventory
  async getInventory(): Promise<{ products: Product[]; variants: unknown[] }> {
    return request<{ products: Product[]; variants: unknown[] }>('/api/admin/inventory');
  },

  async adjustStock(productId?: string, variantId?: string, newStock?: number): Promise<{ message: string }> {
    return request<{ message: string }>('/api/admin/inventory/adjust', {
      method: 'PATCH',
